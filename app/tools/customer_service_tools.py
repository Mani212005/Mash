...
class LookupOrderTool(BaseTool):
    ...
    async def execute(self, **params) -> ToolResult:
        ...
        if phone_number:
            ...
            customer_orders = [
                o for o in _mock_orders.values() if o.get("customer_phone") == phone_number
            ]
            if customer_orders:
                return ToolResult(
                    success=True,
            ...
            return ToolResult(success=False, error="No orders found for this phone number")

        return ToolResult(success=False, error="Please provide either an order ID or phone number")
...
class CheckRefundStatusTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "refund_id": {"type": "string", "description": "The refund request ID if known"},
        },
        "required": ["order_id"],
    }
...
class GetTicketStatusTool(BaseTool):
    ...
    parameters = {
        "type": "object",
        "properties": {"ticket_id": {"type": "string", "description": "The support ticket ID"}},
        "required": ["ticket_id"],
    }
    ...
    async def execute(self, **params) -> ToolResult:
        ...
        if ticket:
            return ToolResult(
                success=True, data=ticket, message=f"Ticket {ticket_id} status: {ticket['status']}"
            )
...
class SearchKnowledgeBaseTool(BaseTool):
    ...
    async def execute(self, **params) -> ToolResult:
        ...
        return ToolResult(success=False, error="No relevant information found in knowledge base")
