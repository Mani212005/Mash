...
from app.services.agent_service import get_agent_orchestrator, Response
    ...
    async def receive_webhook(self, request_data: dict[str, Any]) -> Any:
        ...
        for raw_msg in raw_messages:
            try:
                ...
                response: Response = await self._orchestrator.handle_message(message)
                response_text = response.message

                # 6. Send text response back
                await self.send_text(
                    recipient_id=message.sender_id,
                    text=response_text,
                )
