...
async def list_tickets(status: Optional[str] = Query(None, description="Filter by status")):
    """
...
async def create_ticket(ticket: SupportTicket):
    ...
    try:
        ...
        await state_manager.redis.set(
            ticket_key, json.dumps(ticket.dict()), ex=86400 * 90  # Expire after 90 days
        )
...
async def update_ticket(ticket_id: str, updates: dict):
    ...
    try:
        ...
        # Save back to Redis
        await state_manager.redis.set(
            ticket_key, json.dumps(ticket_dict), ex=86400 * 90  # Expire after 90 days
        )
