...
from app.services.agent_service import Response
...
async def test_receive_webhook_text_message():
    ...
    with (
        ...
        patch.object(
            channel._orchestrator, "handle_message", new_callable=AsyncMock
        ) as mock_handle,
    ):
        ...
        called_msg = mock_handle.call_args[0][0]
        assert isinstance(called_msg, Message)
        assert called_msg.message_id == "msg_123"
        assert called_msg.chat_id == "15550000000"
        assert called_msg.text == "Hello agent"
...
async def test_receive_webhook_audio_message():
    ...
    with (
        ...
        called_msg = mock_handle.call_args[0][0]
        assert isinstance(called_msg, Message)
        assert called_msg.message_id == "msg_456"
        assert called_msg.chat_id == "15550000000"
        assert called_msg.text == "What is the weather like?"
        assert called_msg.transcript == "What is the weather like?"
